function [groups]=sequence_opt(Images_a,group_max_size)
count=0;
groups=[];
for index=1:length(Images_a)
    if index==1
        ...
        if ((currentindex - previousindex) ==1)                    % If the next image is in sequence example 837 - 838 and not crossing *998 - *999
            ...
            if (floor(currentindex/1000)~=floor(previousindex/1000))||(count>group_max_size) % updtae groups and exit 'if' the index crossed *999 or the maximum group size
        ...
        elseif count == 1                       %If next image is not in sequence and the number of images in the group is only =1 
